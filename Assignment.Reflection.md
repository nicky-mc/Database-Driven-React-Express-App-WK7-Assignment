...
Revising the search functionality was another significant task. Initially, the search results were not selectable, and users could not navigate to the detailed view of a post. By updating the `SearchResults.jsx` component, I was able to make the search results in a simple way by searching for any words in the post or tag or in the category. Future update will include HAving an Post author name to improve search. a drop down to slect categories or a navigation. 
#### Theme Selector

One of the features I implemented was a theme selector, allowing users to switch between light and dark modes. This was achieved by maintaining a [`theme`] state in the `App.jsx` file and passing it down as a prop to various components, including `Home.jsx`, The theme state was toggled using a button in the `Navigation.jsx` component, which called a function to switch between light and dark modes. The selected theme was then applied to the body element and various components using CSS classes.

#### Photo Uploads

Implementing photo uploads was another significant feature. In the  component, I used the `multer` middleware on the backend to handle file uploads. The frontend form included a file input element, and the selected image file was appended to a `FormData` object along with other post details. This form data was then sent to the server via a POST request. The server saved the uploaded image to a designated directory and stored the image URL in the database. This allowed users to upload and display images with their posts.
...
THe most complex thing I attempted was an logibn that included an authorisatiuon but sadly the way i tried to implement it wasn't quite right so I reverted it back to my mvp and built from there to where we are now. Any advice welcomed how to implement that. I am hoping my photo upload attempt continues to work.  using urls as i've done in a couple of workshops seems to have done the trick. maybe a more sophiosticated method in future. I also managed a basic search function but would like to learn how to refine that as well as refine an Edit post function as that would completely round off my app. 
Overall I really was inspired by this assignment and ity is one i will continue to develop. 
